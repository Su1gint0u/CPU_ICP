// Board-level regression for tests/grade3/g3_test5.s.
// Exercises byte/half/word loads and stores through the UART-loaded program.

`timescale 1ns / 1ps

module tb_fpga_top_g3_test5;
    ...
    localparam int unsigned BIN_LEN = 220;
    ...
    task automatic uart_monitor;
        ...
        begin
            forever begin
                ...
                $display("[TB_G3T5] UART_TX byte 0x%02h at t=%0t", rx_byte, $time);
            end
        end
    endtask
    ...
    task automatic expect_uart_byte(input logic [7:0] expected_byte, input string label);
        ...
        begin
            ...
            if (uart_seen_q.size() == 0)
                $fatal(1, "[TB_G3T5] Timeout waiting for %s", label);
            got = uart_seen_q.pop_front();
            if (got !== expected_byte)
                $fatal(1, "[TB_G3T5] %s mismatch: expected 0x%02h, got 0x%02h",
                       label, expected_byte, got);
            $display("[TB_G3T5] PASS %s = 0x%02h", label, got);
        end
    endtask
    ...
    initial begin
        ...
        expected = '{
            8'h43, 8'h00, 8'h00, 8'h00, 8'h65, 8'h00, 8'h00, 8'h00,
            8'h87, 8'hff, 8'hff, 8'hff, 8'ha9, 8'hff, 8'hff, 8'hff,
            8'h87, 8'h00, 8'h00, 8'h00, 8'h43, 8'h65, 8'h00, 8'h00,
            8'h87, 8'ha9, 8'hff, 8'hff, 8'h87, 8'ha9, 8'h00, 8'h00,
            8'h43, 8'h65, 8'h87, 8'ha9, 8'h43, 8'h65, 8'h87, 8'ha9,
            8'h02, 8'h00, 8'h02
        };
        ...
        fd = $fopen("/media/alice/workplace/CPU_ICP/tests/grade3/artifacts/g3_test5/test.bin", "rb");
        if (fd == 0)
            $fatal(1, "[TB_G3T5] Could not open g3_test5 test.bin");
        nread = $fread(bin_bytes, fd);
        $fclose(fd);
        if (nread != BIN_LEN)
            $fatal(1, "[TB_G3T5] Expected %0d bytes, read %0d", BIN_LEN, nread);
        ...
        $display("[TB_G3T5] Sending tests/grade3/artifacts/g3_test5/test.bin");
        uart_send_frame(32'h8000_0000, BIN_LEN);

        for (i = 0; i < 43; i++)
            expect_uart_byte(expected[i], $sformatf("byte[%0d]", i));

        $display("[TB_G3T5] PASS g3_test5 board-level load/store regression");
        $finish;
    end
