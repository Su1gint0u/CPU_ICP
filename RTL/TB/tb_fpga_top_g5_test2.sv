// Board-level regression for tests/grade5/g5_test2.s.
// Exercises RV32F integer conversions and division through UART download.

`timescale 1ns / 1ps

module tb_fpga_top_g5_test2;
    ...
    localparam int unsigned BIN_LEN          = 408;
    localparam int unsigned EXPECT_LEN       = 43;
    ...
    logic [7:0]  expected [0:EXPECT_LEN-1];
    ...
    task automatic uart_monitor;
        ...
        begin
            forever begin
                ...
                $display("[TB_G5T2] UART_TX byte 0x%02h at t=%0t", rx_byte, $time);
            end
        end
    endtask
    ...
    task automatic expect_uart_byte(input logic [7:0] expected_byte, input string label);
        ...
        begin
            ...
            if (uart_seen_q.size() == 0)
                $fatal(1, "[TB_G5T2] Timeout waiting for %s", label);
            got = uart_seen_q.pop_front();
            if (got !== expected_byte)
                $fatal(1, "[TB_G5T2] %s mismatch: expected 0x%02h, got 0x%02h",
                       label, expected_byte, got);
            $display("[TB_G5T2] PASS %s = 0x%02h", label, got);
        end
    endtask
    ...
    initial begin
        ...
        expected = '{
            8'h28, 8'h00, 8'h00, 8'h00,
            8'he2, 8'hff, 8'hff, 8'hff,
            8'hda, 8'hff, 8'hff, 8'hff,
            8'h9a, 8'hff, 8'hff, 8'hff,
        ...
        fd = $fopen("/media/alice/workplace/CPU_ICP/tests/grade5/artifacts/g5_test2/test.bin", "rb");
        if (fd == 0)
            $fatal(1, "[TB_G5T2] Could not open g5_test2 test.bin");
        nread = $fread(bin_bytes, fd);
        $fclose(fd);
        if (nread != BIN_LEN)
            $fatal(1, "[TB_G5T2] Expected %0d bytes, read %0d", BIN_LEN, nread);
        ...
        $display("[TB_G5T2] Sending tests/grade5/artifacts/g5_test2/test.bin");
        uart_send_frame(32'h8000_0000, BIN_LEN);

        for (i = 0; i < EXPECT_LEN; i++)
            expect_uart_byte(expected[i], $sformatf("byte[%0d]", i));

        $display("[TB_G5T2] PASS g5_test2 board-level FP conversion regression");
        $finish;
    end
