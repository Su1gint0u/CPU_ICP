// Testbench for the board-facing Nexys4/test_add.bin image.
// Sends the same binary that send_frame.py typically uses and checks that the
// CPU produces UART output before the bridge response.

`timescale 1ns / 1ps

module tb_fpga_top_test_add;
    ...
    localparam int unsigned BIN_LEN = 144;
    ...
    task automatic uart_monitor;
        ...
        begin
            forever begin
                ...
                $display("[TB_BIN] UART_TX byte 0x%02h at t=%0t", rx_byte, $time);
            end
        end
    endtask
    ...
    task automatic expect_uart_byte(input logic [7:0] expected, input string label);
        ...
        begin
            ...
            if (uart_seen_q.size() == 0)
                $fatal(1, "[TB_BIN] Timeout waiting for %s", label);
            got = uart_seen_q.pop_front();
            if (got !== expected)
                $fatal(1, "[TB_BIN] %s mismatch: expected 0x%02h, got 0x%02h",
                       label, expected, got);
            $display("[TB_BIN] PASS %s = 0x%02h", label, got);
        end
    endtask
    ...
    initial begin
        ...
        fd = $fopen("/media/alice/workplace/CPU_ICP/Nexys4/test_add.bin", "rb");
        if (fd == 0)
            $fatal(1, "[TB_BIN] Could not open test_add.bin");
        nread = $fread(bin_bytes, fd);
        $fclose(fd);
        if (nread != BIN_LEN)
            $fatal(1, "[TB_BIN] Expected %0d bytes, read %0d", BIN_LEN, nread);
        ...
        $display("[TB_BIN] Sending Nexys4/test_add.bin");
        uart_send_frame(32'h8000_0000, BIN_LEN);

        expect_uart_byte(8'h41, "test_add output A");
        expect_uart_byte(8'h42, "test_add output B");
        expect_uart_byte(8'h31, "test_add output 1");
        expect_uart_byte(8'h43, "test_add output C");
        expect_uart_byte(8'h02, "response cmd");
        expect_uart_byte(8'h00, "response status");
        expect_uart_byte(8'h02, "response checksum");

        $display("[TB_BIN] PASS Nexys4/test_add.bin board image");
        $finish;
    end
